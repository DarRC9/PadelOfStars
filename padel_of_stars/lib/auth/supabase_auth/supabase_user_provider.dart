...
class PadelOfStarsSupabaseUser extends BaseAuthUser {
  PadelOfStarsSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> padelOfStarsSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PadelOfStarsSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
